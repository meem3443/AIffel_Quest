...
class Aiffel:
    ...
    def show(self): #Q2 현재 총점및 정보 모두 개인 출력
        print(f'이름 : {self.name} 학번 : {self.code} 점수 : {self.score} 페널티 : {self.penalty} 총점 : {self.total}')    
    ...
    def grouping(self, groupSize): #Q4 그루핑 기능 큐를 사용한다. 일단 섞어서 큐에 넣고 pop통해서 값을 가져온다. 이후 남은인원이 그룹사이즈보다 작으면 전부 거기다가 추가한다.
...
def main():
    ...
    a.deleteMan('2431100') #특정 학번을 지웁니다. 이름은 겹칠 수 있기에 학번으로 지정합니다.
    
    a.showAll('default') #total, penalty, score가 아니라면 무조건 넣은순서대로 출력합니다.
